...
class Profile_model extends CI_Model {
    ...
    private function _get_datatables_query() {
        ...
        $this->db->join('bank', 'bank.id = users_bank.BankId', 'left');
    ...
    function getProfileById($userId) {
        ...
        $sql = "SELECT 
                  ...
                  FROM
                    users a 
                    INNER JOIN users_profile b 
                      ON a.id = b.UserId 
                    LEFT JOIN provinces c 
                      ON b.Province = c.id 
                    LEFT JOIN regencies d 
                      ON b.City = d.id 
                    LEFT JOIN users_emergency_contacts e 
                      ON b.UserId = e.user_id 
                    LEFT JOIN users_works f 
                      ON b.UserId = f.user_id 
                    LEFT JOIN users_list_of_assets g 
                      ON b.UserId = g.user_id 
    ...
    function getImageBy($id) {
        ...
        $query = $this->db->get('users_pictures');
        return $query->row();
    }

    function getUserEmergencyById($id) {
        $this->db->where('user_id', $id);
        $query = $this->db->get('users_emergency_contacts');
        return $query->row();
    }

    function getUserWorkById($id) {
        $this->db->where('user_id', $id);
        $query = $this->db->get('users_works');

        if ($query->num_rows() > 0)
            return $query->row();
    }

    function getMasterIncomePerYear() {
        $query = $this->db->get('income_per_years');

        if ($query->num_rows() > 0)
            return $query->result_array();
    }

    function getUserListAssetById($id) {
        $this->db->where('user_id', $id);
        $query = $this->db->get('users_list_of_assets');

        if ($query->num_rows() > 0)
            return $query->row();
    }
    
    function getUserPictureId($id) {
        $this->db->where('user_id', $id);
        $query = $this->db->get('users_pictures');
